...
Public Class ViewConsignmentShipping
    ...
    Private Sub ViewConsignmentShipping_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub ClearDataInDatagrid()
        dgvConsignmentShipping.DataSource = Nothing
    End Sub
    ...
    Public Sub ShowDataByFilters()
        ...
        If cboWarehouse.Text <> "" Then
            WarehouseFilter = " AND CustomerClass = '" + cboWarehouse.Text + "'"
        Else
            WarehouseFilter = ""
        End If
        If txtTextFilter.Text <> "" Then
            TextFilter = " AND (PartDescription LIKE '%" + usefulFunctions.checkQuote(txtTextFilter.Text) + "%' OR PartNumber LIKE '%" + usefulFunctions.checkQuote(txtTextFilter.Text) + "%')"
        Else
            TextFilter = ""
        End If
        If chkUseDateRange.Checked = True Then
            BeginDate = dtpBeginDate.Text
            EndDate = dtpEndingDate.Text
        ...
        cmd = New SqlCommand("SELECT * FROM ConsignmentShippingTable WHERE DivisionID = @DivisionID" + PartFilter + TextFilter + WarehouseFilter + DateFilter, con)
        ...
        myAdapter.Fill(ds, "ConsignmentShippingTable")
        dgvConsignmentShipping.DataSource = ds.Tables("ConsignmentShippingTable")
        con.Close()
    End Sub
    ...
    Public Sub LoadFOB()
        cmd = New SqlCommand("SELECT FOBID FROM FOBTable WHERE DivisionID = @DivisionID AND Status = @Status ORDER BY FOBID", con)
        cmd.Parameters.Add("@DivisionID", SqlDbType.VarChar).Value = cboDivisionID.Text
        ...
        cboWarehouse.DataSource = ds3.Tables("FOBTable")
        con.Close()
        cboWarehouse.SelectedIndex = -1
    End Sub

    Public Sub LoadFOBCanadian()
        cmd = New SqlCommand("SELECT FOBID FROM FOBTable WHERE DivisionID = @DivisionID AND Status = @Status AND Type = @Type ORDER BY FOBID", con)
        cmd.Parameters.Add("@DivisionID", SqlDbType.VarChar).Value = cboDivisionID.Text
        cmd.Parameters.Add("@Status", SqlDbType.VarChar).Value = "ACTIVE"
    ...
    Public Sub LoadTotals()
        ...
        Dim TotalAmountStatement As String = "SELECT SUM(ExtendedAmount) FROM ConsignmentShippingTable WHERE DivisionID = @DivisionID" + PartFilter + TextFilter + WarehouseFilter + DateFilter
        ...
        Dim TotalQuantityStatement As String = "SELECT SUM(QuantityShipped) FROM ConsignmentShippingTable WHERE DivisionID = @DivisionID" + PartFilter + TextFilter + WarehouseFilter + DateFilter
        ...
        End Try
        Try
            TotalAmount = CDbl(TotalAmountCommand.ExecuteScalar)
        Catch ex As Exception
            TotalAmount = 0
        End Try
        con.Close()

        txtTotalAmount.Text = FormatCurrency(TotalAmount, 2)
    ...
    Public Sub ClearData()
        ...
        dtpBeginDate.Text = Today.Date.ToString()
        dtpEndingDate.Text = Today.Date.ToString()
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click
        ...
        Using NewPrintConsignmentShipping As New PrintConsignmentShipping
            Dim Result = NewPrintConsignmentShipping.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub dgvConsignmentShipping_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvConsignmentShipping.CellDoubleClick
        Dim RowShipmentNumber As Integer

        Dim RowIndex As Integer = Me.dgvConsignmentShipping.CurrentCell.RowIndex

        RowShipmentNumber = Me.dgvConsignmentShipping.Rows(RowIndex).Cells("ShipmentNumberColumn").Value
