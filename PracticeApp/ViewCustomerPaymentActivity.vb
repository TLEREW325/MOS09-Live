...
Public Class ViewCustomerPaymentActivity
    Inherits System.Windows.Forms.Form

    Dim TextFilter, CustomerFilter, DateFilter, CheckFilter, PaymentAmountFilter, PaymentTypeFilter, InvoiceFilter As String
    Dim InvoiceNumber, PaymentAmount As Integer
    Dim strInvoiceNumber, strPaymentAmount As String
    ...
    Private Sub ViewCustomerPaymentActivity_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub dgvCustomerPayments_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvCustomerPayments.CellClick
        If (e.RowIndex >= 0) Then
            Dim RowIndex As Integer = Me.dgvCustomerPayments.CurrentCell.RowIndex
            PaymentBatchNumber.batchNumber = Me.dgvCustomerPayments.Rows(RowIndex).Cells("BatchNumber").Value
            PaymentBatchNumber.PaymentID = Me.dgvCustomerPayments.Rows(RowIndex).Cells("PaymentID").Value
        ...
        End If
        Try
            For Each row As DataGridViewRow In dgvCustomerPayments.Rows

                Dim pdfname As String = row.Cells("PaymentID").Value.ToString
                Dim CashReceiptExists As String = "\\TFP-FS\CashReceipts\UploadedCashReceipts\" + pdfname + ".pdf"
                Dim batchName As String = row.Cells("BatchNumber").Value.ToString
                Dim CashBatchExists As String = "\\TFP-FS\CashReceipts\UploadedCashBatch\" + batchName + ".pdf"

                If File.Exists(CashReceiptExists) And File.Exists(CashBatchExists) Then

                    row.Cells("InvoiceNumberColumn").Style.BackColor = Color.LightGreen

                ElseIf File.Exists(CashReceiptExists) Then

                    row.Cells("InvoiceNumberColumn").Style.BackColor = Color.LightBlue

                ElseIf File.Exists(CashBatchExists) Then


                    row.Cells("InvoiceNumberColumn").Style.BackColor = Color.LightYellow

                Else

                    row.Cells("InvoiceNumberColumn").Style.BackColor = Color.LightCoral
    ...
    Private Sub dgvCustomerPayments_CellDoubleClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles dgvCustomerPayments.CellDoubleClick
        Dim RowCustomer As String
        Dim RowIndex As Integer = Me.dgvCustomerPayments.CurrentCell.RowIndex

        RowCustomer = Me.dgvCustomerPayments.Rows(RowIndex).Cells("CustomerIDColumn").Value
    ...
    Private Sub dgvCustomerPayments_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvCustomerPayments.CellValueChanged
        Dim RowDivisionID, CheckNumber, PaymentType, CardNumber, CardDate, AuthorizationNumber, ReferenceNumber, CheckComment, CreditComment As String
        Dim ARTransactionKey As Integer

        If Me.dgvCustomerPayments.RowCount = 0 Then
            'Skip if datagrid is blank
        Else
            Dim RowIndex As Integer = Me.dgvCustomerPayments.CurrentCell.RowIndex

            Try
                ARTransactionKey = Me.dgvCustomerPayments.Rows(RowIndex).Cells("ARTransactionKeyColumn").Value
            Catch ex As Exception
                ARTransactionKey = 0
            End Try
            Try
                PaymentType = Me.dgvCustomerPayments.Rows(RowIndex).Cells("PaymentTypeColumn").Value
            Catch ex As Exception
                PaymentType = ""
            End Try
            Try
                CheckNumber = Me.dgvCustomerPayments.Rows(RowIndex).Cells("CheckNumberColumn").Value
            Catch ex As Exception
                CheckNumber = ""
            End Try
            Try
                CardNumber = Me.dgvCustomerPayments.Rows(RowIndex).Cells("CardNumberColumn").Value
            Catch ex As Exception
                CardNumber = ""
            End Try
            Try
                CardDate = Me.dgvCustomerPayments.Rows(RowIndex).Cells("CardDateColumn").Value
            Catch ex As Exception
                CardDate = ""
            End Try
            Try
                AuthorizationNumber = Me.dgvCustomerPayments.Rows(RowIndex).Cells("AuthorizationNumberColumn").Value
            Catch ex As Exception
                AuthorizationNumber = ""
            End Try
            Try
                CheckComment = Me.dgvCustomerPayments.Rows(RowIndex).Cells("CheckCommentColumn").Value
            Catch ex As Exception
                CheckComment = ""
            End Try
            Try
                ReferenceNumber = Me.dgvCustomerPayments.Rows(RowIndex).Cells("ReferenceNumberColumn").Value
            Catch ex As Exception
                ReferenceNumber = ""
            End Try
            Try
                CreditComment = Me.dgvCustomerPayments.Rows(RowIndex).Cells("CreditCommentColumn").Value
            Catch ex As Exception
                CreditComment = ""
            End Try
            Try
                RowDivisionID = Me.dgvCustomerPayments.Rows(RowIndex).Cells("DivisionIDColumn").Value
            Catch ex As Exception
                RowDivisionID = ""
            End Try

            If ARTransactionKey = 0 Then
                'Skip save procedure
            Else
    ...
    Public Sub ClearDataInDatagrid()
        dgvCustomerPayments.DataSource = Nothing
    End Sub

    Public Sub ShowDataFilters()
        ...
        If cboPaymentType.Text <> "" Then
            PaymentTypeFilter = " AND PaymentType = '" + cboPaymentType.Text + "'"
        Else
            PaymentTypeFilter = ""
        End If
        If txtPaymentAmount.Text <> "" Then
            PaymentAmount = Val(txtPaymentAmount.Text)
            strPaymentAmount = CStr(PaymentAmount)
            PaymentAmountFilter = " AND PaymentAmount = '" + strPaymentAmount + "'"
        Else
            PaymentAmountFilter = ""
        End If
        ...
        Else
            ...
            ElseIf chkPayment.Checked = True Then
        ...
        If cboDivisionID.Text = "ADM" Then
            cmd = New SqlCommand("SELECT * FROM ARCustomerPaymentQuery WHERE DivisionID <> @DivisionID" + CustomerFilter + CheckFilter + TextFilter + PaymentTypeFilter + PaymentAmountFilter + InvoiceFilter + DateFilter + " ORDER BY DivisionID, CustomerID", con)
            ...
            dgvCustomerPayments.DataSource = ds.Tables("ARCustomerPaymentQuery")
            con.Close()
            Me.dgvCustomerPayments.Columns("DivisionIDColumn").Visible = True
        Else
            cmd = New SqlCommand("SELECT * FROM ARCustomerPaymentQuery WHERE DivisionID = @DivisionID" + CustomerFilter + CheckFilter + TextFilter + PaymentTypeFilter + PaymentAmountFilter + InvoiceFilter + DateFilter + " ORDER BY CustomerID", con)
            ...
            dgvCustomerPayments.DataSource = ds.Tables("ARCustomerPaymentQuery")
            con.Close()
            Me.dgvCustomerPayments.Columns("DivisionIDColumn").Visible = False
        End If
    End Sub
    ...
    Public Sub LoadPaymentType()
        cmd = New SqlCommand("SELECT DISTINCT PaymentType FROM ARCustomerPaymentQuery WHERE DivisionID = @DivisionID", con)
        cmd.Parameters.Add("@DivisionID", SqlDbType.VarChar).Value = cboDivisionID.Text
        If con.State = ConnectionState.Closed Then con.Open()
        ds3 = New DataSet()
        myAdapter3.SelectCommand = cmd
        myAdapter3.Fill(ds3, "ARCustomerPaymentQuery")
        cboPaymentType.DataSource = ds3.Tables("ARCustomerPaymentQuery")
        con.Close()
        cboPaymentType.SelectedIndex = -1
    End Sub
    ...
    Public Sub LoadTotalPaymentsByFilters()
        If cboCustomerID.Text <> "" Then
            CustomerFilter = " AND CustomerID = '" + usefulFunctions.checkQuote(cboCustomerID.Text) + "'"
        Else
            CustomerFilter = ""
        ...
        If cboDivisionID.Text = "ADM" Then
            Dim TotalPaymentsString As String = "SELECT SUM(PaymentAmount) FROM ARCustomerPaymentQuery WHERE DivisionID <> @DivisionID" + CustomerFilter + CheckFilter + TextFilter + PaymentTypeFilter + PaymentAmountFilter + InvoiceFilter + DateFilter
            Dim TotalPaymentsCommand As New SqlCommand(TotalPaymentsString, con)
            TotalPaymentsCommand.Parameters.Add("@DivisionID", SqlDbType.VarChar).Value = "ADM"
            TotalPaymentsCommand.Parameters.Add("@BeginDate", SqlDbType.VarChar).Value = BeginDate
            TotalPaymentsCommand.Parameters.Add("@EndDate", SqlDbType.VarChar).Value = EndDate
        ...
        Else
            Dim TotalPaymentsString As String = "SELECT SUM(PaymentAmount) FROM ARCustomerPaymentQuery WHERE DivisionID = @DivisionID" + CustomerFilter + CheckFilter + TextFilter + PaymentTypeFilter + PaymentAmountFilter + InvoiceFilter + DateFilter
            Dim TotalPaymentsCommand As New SqlCommand(TotalPaymentsString, con)
            TotalPaymentsCommand.Parameters.Add("@DivisionID", SqlDbType.VarChar).Value = cboDivisionID.Text
            TotalPaymentsCommand.Parameters.Add("@BeginDate", SqlDbType.VarChar).Value = BeginDate
            TotalPaymentsCommand.Parameters.Add("@EndDate", SqlDbType.VarChar).Value = EndDate
        ...
        txtTotalPayments.Text = FormatCurrency(TotalPayments, 2)
    End Sub
    ...
    Private Sub chkInvoice_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkInvoice.CheckedChanged
        If chkInvoice.Checked = True Then
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click
        GDS = ds
        Using NewPrintCustomerPaymentFiltered As New PrintCustomerPaymentFiltered
            Dim Result = NewPrintCustomerPaymentFiltered.ShowDialog()
        End Using
    End Sub
    ...
    Private Sub dgvCustomerPayments_Sorted(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvCustomerPayments.Sorted
        Try
            For Each row As DataGridViewRow In dgvCustomerPayments.Rows
    ...
    Private Sub ScanToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ScanToolStripMenuItem.Click
        ...
        If File.Exists(W9Exists) Then
            ...
            For Each row As DataGridViewRow In dgvCustomerPayments.Rows
                Dim pdfname2 As String = row.Cells("PaymentID").Value.ToString
    ...
    Private Sub UploadToolStripMenuItem3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UploadToolStripMenuItem3.Click
