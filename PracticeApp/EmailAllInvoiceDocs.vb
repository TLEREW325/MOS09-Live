...
Public Class EmailAllInvoiceDocs
    ...
    Private Sub CRInvoiceViewer_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles CRInvoiceViewer.Load
        ...
        If EmailShipmentNumber = 0 Or CertCount = 0 Or CheckForCerts = 0 Then
            ...
            mail.Subject = EmailInvoiceCustomer + " - #" + GlobalDivisionCode + strInvoiceNumber
            ...
            If GlobalPrintNoCertPage = "YES" And CertCount <> 0 Then
                mail.Attachments.Add("\\TFP-FS\TransferData\ExportedCerts\NO-CERT.pdf")
            End If

            'adding attachment
            mail.Attachments.Add("\\TFP-FS\TransferData\TruweldInvoices\" & InvoiceName & ".pdf")
        ...
        Else
            ...
            'adding body message information to the mail message
            If EmailAddress <> "" Then
                mail.Body = "This is an Invoice/Certs for an order placed with Truweld / TFP Corporation. Email to --  " + EmailCustomerEmailAddress
            Else
                'Dont add this line
            End If
            ...
            'adding attachment
            mail.Attachments.Add("\\TFP-FS\TransferData\TruweldInvoices\" & CertName & ".pdf")
            mail.Attachments.Add("\\TFP-FS\TransferData\TruweldInvoices\" & InvoiceName & ".pdf")
